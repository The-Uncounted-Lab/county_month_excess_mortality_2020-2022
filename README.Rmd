...
variableDescriptions <-
  list(
    ...
    ratioMean = glue::glue(
      "Mean ratio of excess deaths to COVID-19 deaths, calculated as (excDeathsMean / COVIDDeathsMean)."
    ),
    ratioLow = glue::glue(
      "Lower 5th percentile of ratio of excess deaths to COVID-19 deaths, calculated as (excDeathsLow / COVIDDeaths)."
    ),
    ratioMed = glue::glue(
      "Median ratio of excess deaths to COVID-19 deaths, calculated as (excDeathsMed / COVIDDeaths)."
    ),
    ratioUp = glue::glue(
      "Upper 95th percentile of ratio of excess deaths to COVID-19 deaths, calculated as (excDeathsUp / COVIDDeaths)."
    ),
    probPositiveExcess = glue::glue(
      "Probability that excess deaths exceeds zero."
    ),
    probPositiveNCExcess = glue::glue(
      "Probability that excess deaths not assigned to COVID-19 exceeds zero."
    ),
    probPositive = glue::glue(
      "Probability that excess deaths exceeds zero."
    ),
    probRatioLargerThan1 = glue::glue(
      "Probability that the ratio of excess deaths to COVID-19 deaths exceeds 1."
    )
  )
```
...
autoDictionary <- function(.file_path, 
  ...
  vtable::vtable(
    ...
    lush = TRUE,
    desc = "",
    summ = c('mean(x)', 'median(x)', 'sd(x)', 'nuniq(x)'),
    ...
  ) %>% 
...
fileNames <- list.files(
  here::here("data", "output", "estimates"),
  pattern = ".csv",
  full.names = TRUE,
  recursive = TRUE
)
