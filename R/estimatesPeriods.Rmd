---
title: "Estimates of Excess Deaths by 4 Month Periods"
...
here::i_am('R/estimatesPeriods.Rmd')
...
COVIDPeriodsData <- tibble(arrow::read_feather(here::here(outDir,'COVIDPeriodsData.feather')))
...
ACData <- ACData %>%
  ...
  mutate(period = factor(
    case_when(
           between(monthYear,
                   make_date(2020,3,1),
                   make_date(2020,6,1)) ~ 1,
           between(monthYear,
                   make_date(2020,7,1),
                   make_date(2020,10,1)) ~ 2,
           between(monthYear,
                   make_date(2020,11,1),
                   make_date(2021,2,1)) ~ 3,
           between(monthYear,
                   make_date(2021,3,1),
                   make_date(2022,6,1)) ~ 4,
           between(monthYear,
                   make_date(2021,7,1),
                   make_date(2021,10,1)) ~ 5,
           between(monthYear,
                   make_date(2021,11,1),
                   make_date(2022,2,1)) ~ 6),
    levels=1:6,
    labels=c('(Mar-Jun 2020)','(Jul-Oct 2020)','(Nov 2020 - Feb 2021)',
             '(Mar-Jun 2021)','(Jul-Oct 2021)','(Nov 2021 - Feb 2022)')))
...
finalSimsFIPS <- finalSimsFIPS %>%
  mutate(periodNum = as.integer(period)) %>%
  left_join(COVIDPeriodsData,by=c('FIPSCode','stateFIPS','periodNum'='period'))
...
finalSimsFIPS %>% write_csv(here::here(outDir,'exMortEstimatesPeriodsINLA.csv'))
