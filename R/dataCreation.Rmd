...
ACData <- ACData %>% 
  mutate(FIPSCode = coalesce(FIPSCode, FIPSCode2021),
         countyName = coalesce(countyName,countyName2021))
...
simData <- crossing(year=2015:2021,
                    month=1:12,
                    FIPSCode=FIPSCodes)

simData <- simData %>%
  mutate(imputedDeaths = round(rbeta(nrow(simData),2,2)*9))

simDataYearly <- crossing(year=2010:2021,
                          FIPSCode=FIPSCodes)

simDataYearly <- simDataYearly %>%
  mutate(imputedDeaths = round(rbeta(nrow(simDataYearly),2,2)*9))
