...
monthlyModelErrors <- simDataMonthly %>% 
  ...
  group_by(FIPSCode) %>%
  summarize(meanExpDeathsM = mean(expDeaths,na.rm=T),
            meanDeathsM = mean(deaths,na.rm=T),
            meanDifferenceM = abs(meanExpDeathsM - meanDeathsM)/meanDeathsM)
...
png(here::here('figures','nationalTrend.png'), width = 3000, height = 2000, res = 300, pointsize=4)
