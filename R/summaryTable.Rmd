...
exMortEstimatesAC <- read_csv(here::here(outDir,'exMortEstimatesACPI.csv'))
exMortEstimatesNC <- read_csv(here::here(outDir,'exMortEstimatesNCPI.csv'))
...
exMortEstimatesNC <- exMortEstimatesNC %>%
  left_join(metro, by = 'FIPSCode')
...
tableDataNC <- tableDataNC %>%
  arrange(BEARegion,metroName) %>%
  slice(45,41:44,5,1:4,
        10,6:9,15,11:14,
        20,16:19,25,21:24,
        30,26:29,35,31:34,
        40,36:39)
...
tableDataNC %>%
  ...
  kable(col.names = c(c('BEA Region','Metro Status'),
        ...
        digits = c(0,0,rep(c(0,0,3,0,0),2)),
        booktabs=T,
        linesep=c(rep('',3),'\\addlinespace[2pt]','\\addlinespace[5pt]'),
        format.args = list(big.mark = ','),
        caption = 'Summary Table - Natural Causes') %>%
  add_header_above(c(" " = 2, rep(c("Number of Deaths" = 3, "Rates per 100,000 PY" = 2),2))) %>%
  add_header_above(c(" " = 2, "2020" = 5, "2021" = 5)) %>%
  kable_styling(font_size = 7.5)
