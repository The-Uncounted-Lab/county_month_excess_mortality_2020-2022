---
title: "Estimates of Excess Deaths"
...
load(here::here('R','RObjects','modelBYMLinearTimeINLA.RData'))
...
testFigure <- testData %>%
  ...
  geom_line(mapping=aes(x=monthYear,y=expDeathsMean,color='Expected')) +
...
load(here::here('R','RObjects','modelYBYMLinearTimeINLA.RData'))
...
draws.y <- inla.posterior.sample(n.sim, model.Y.BYM.LinearTime.INLA)
model.names <- row.names(draws.y[[1]]$latent)
y.names <- grep("Predictor", model.names)

rate.draws.y <- sapply(draws.y, function(x) exp(x$latent[y.names]))
...
save(death.draws,death.draws.y,file=here::here('R','RObjects','simulations.RData'))
