...
COVID_MetroRegion <- COVID_MetroDivision %>% 
...
metroRegionMonth <- simDF %>%
  ...
  left_join(
    COVID_MetroRegion,
    by = c("census_region", "metroCat3", "monthYear")
  ) %>%
  mutate(
    across(
      c(starts_with("V") & ends_with("Exc")),
  ...
  mutate(
    ...
    across(
      c(starts_with("V") & ends_with("ExcCumulative")),
      .fns = c(ExcessCOVIDRatio = ~ .x / totalCOVIDDeathsCumulative),
      .names = "{.col}{.fn}"
    )
  ) %>%
  ungroup() %>%
  ...
  quantile_interval(
    c_across(starts_with("V") & ends_with("ExcessCOVIDRatio")), ExcessCOVIDRatioCumulative
  ) %>% 
  quantile_interval(
    c_across(starts_with("V") & ends_with("SumCumulative")), expCumulative
  ) %>%
...
# Facet line graphs of excess vs COVID vs non-COVID excess
```{r excess v covid data prep}
metroDivisionMonthPopRates <- metroDivisionMonth %>% 
  mutate(
    across(
      c(totalCOVIDDeaths, excMed, excNCMed),
    .fns = c(
      `100k` = ~ 100000 * .x / pop
    )
  )
  ...
  pivot_longer(
    cols = c(ends_with("_100k")),
    names_to = "outcome"
  ) %>% 
  mutate(
    across(
      c(excLow, excUp, excNCLow, excNCUp),
    .fns = c(
      `100k` = ~ 100000 * .x / pop
    )
  )
...
excessVsCOVIDPlot <- metroDivisionMonthPopRates %>%
  ggplot(
    aes(
      x = monthYear,
      y = value,
...
cairo_pdf(
  filename = here::here("figures", "excessVsCOVIDPlot.pdf"),
  width = 10,
  height = 14
)
excessVsCOVIDPlot
...
ExcessCOVIDRatioCumulativePlot <- gExcessCOVIDRatioCumulative +
  ...
  labs(
    y = "Ratio of excess deaths to COVID-19 deaths (cumulative)",
    x = NULL,
    color = "group",
    linetype = "group"
  ) +
...
#   filename = here::here("figures", "excessCumulative.png"),
cairo_pdf(
  filename = here::here("figures", "cumulative_excess_to_covid_ratio.pdf"),
  width = 7.8,
  height = 4.55
)
