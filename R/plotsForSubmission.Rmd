...
quantile_interval <- function(.x,
  ...
  .x |>
    mutate(
      "{{ .newVarName }}Low" := quantile({{ .vars }}, probs = low, na.rm = na.rm),
      "{{ .newVarName }}Med" := quantile({{ .vars }}, probs = med, na.rm = na.rm),
      "{{ .newVarName }}Up" := quantile({{ .vars }}, probs = up, na.rm = na.rm)
    )
}
