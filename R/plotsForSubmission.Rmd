...
```{r import data}
monthlyEstimates <- read_csv(file.path(outDir, "estimates", "estimatesMonthlyFull.csv")) %>% 
  mutate(monthYear = make_date(year = year, month = month))
monthlyEstimatesState <- read_csv(file.path(outDir, "estimates", "estimatesStateMonths.csv")) %>% 
...
# filter to first 24 pandemic months and add "sig" flag and pop
