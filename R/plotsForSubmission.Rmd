...
# MCD
COVID_StateRaw_MCD <- data.table::fread(
        file.path(inDir, 'CDC', 'COVIDMCDData', 'monthly_state_covid_mcd.txt'),
        na.strings = c("Missing", "Suppressed", "Not Applicable"),
        keepLeadingZeros = TRUE,
      )

COVID_MetroDivisionRaw_MCD <- list.files(
      file.path(inDir, 'CDC', 'COVIDMCDData', 'metroDivisionTotals'),
      pattern = "*.txt",
      full.names = TRUE
    ) %>%
    map_dfr(
...
COVID_MetroRegionRaw_MCD <- list.files(
      file.path(inDir, 'CDC', 'COVIDMCDData', 'metroRegion'),
      pattern = "*.txt",
      full.names = TRUE
    ) %>%
    map_dfr(
...
# metro-month
COVID_MetroRaw_MCD <- list.files(
      file.path(inDir, 'CDC', 'COVIDMCDData', 'metroTotals'),
      pattern = "*.txt",
      full.names = TRUE
    ) %>%
    map_dfr(
...
# division-month
COVID_DivisionRaw_MCD <- data.table::fread(
        file.path(inDir, 'CDC', 'COVIDMCDData', 'monthly_division_covid_mcd.txt'),
        na.strings = c("Missing", "Suppressed", "Not Applicable"),
        keepLeadingZeros = TRUE,
      )
...
NC_MetroRaw <- list.files(
      file.path(inDir, 'CDC', 'mortalityNCData', 'metroTotals'),
      pattern = "*.txt",
      full.names = TRUE
    ) %>%
    map_dfr(
...
NC_DivisionRaw <- data.table::fread(
  file.path(inDir, 'CDC', 'mortalityNCData', 'monthly_division_natural.txt'),
  na.strings = c("Missing", "Suppressed", "Not Applicable"),
  keepLeadingZeros = TRUE,
  )
...
# MCD
COVID_MetroRegion_MCD <- COVID_MetroRegionRaw_MCD %>% 
  mutate(
    metroCat3 = str_match(file, 'monthly_([:alpha:]+)_metro.*')[,2],
    metroCat3 = case_when(
      metroCat3 == 'large' ~ 'Large Metro',
      metroCat3 == 'medsmall' ~ 'Medium or Small Metro',
      metroCat3 == 'non' ~ 'Non Metro'
    ),
    census_region = str_match(`Residence Census Region`,'Census Region \\d{1}: ([\\w\\s]+)')[,2]) %>%
  separate(`Month Code`, into=c('year', 'month'), sep='/', convert = T) %>%
  mutate(monthYear = make_date(year, month)) %>%
...
NC_Metro <- NC_MetroRaw %>% 
  ...
  separate(`Month Code`, into=c('year', 'month'), sep='/', convert = T) %>%
  mutate(monthYear = make_date(year, month)) %>%
  select(metroCat3, monthYear, totalDeaths = Deaths) %>% 
  filter(!is.na(monthYear))

NC_Division <- NC_DivisionRaw %>% 
  separate(`Month Code`, into = c("year", "month"), sep = "/", convert = T) %>%
  mutate(
  ...
  select(census_division, monthYear, totalDeaths = Deaths) %>% 
  filter(!is.na(monthYear), !is.na(census_division))
...
### combined MCD
deaths_MetroRegion_MCD <- inner_join(COVID_MetroRegion_MCD, NC_MetroRegion) %>% 
  filter(monthYear >= make_date(2020, 3))
...
COVID_MetroDivision_MCD <- COVID_MetroDivisionRaw_MCD %>%
    separate(`Month Code`, into = c("year", "month"), sep = "/", convert = T) %>%
  mutate(
    monthYear = make_date(year, month),
    metroCat = stringr::str_extract(
      file,
      "(large_metro|medsmall_metro|non_metro)"
    ),
    metroCat3 = case_when(
      metroCat == "large_metro" ~ "Large Metro",
      metroCat == "medsmall_metro" ~ "Medium or Small Metro",
      metroCat == "non_metro" ~ "Non Metro"
    ),
    census_division = str_match(
      `Residence Census Division`,
      "Division \\d{1}: ([\\w\\s]+)"
  ...
  filter(!is.na(census_division), !is.na(monthYear)) %>% 
  group_by(census_division, metroCat3, monthYear) %>% 
  summarize(
    totalCOVIDDeaths = sum(Deaths),
    .groups = "drop"
...
#### MCD
COVID_State_MCD <- COVID_StateRaw_MCD %>% 
  separate(`Month Code`, into = c("year", "month"), sep = "/", convert = T) %>%
  mutate(monthYear = make_date(year, month)) %>%
  select(state = `Residence State`, stateFIPS = `Residence State Code`, monthYear, totalCOVIDDeaths = Deaths)
  
### metro-month MCD
COVID_Metro_MCD <- COVID_MetroRaw_MCD %>% 
  separate(`Month Code`, into = c("year", "month"), sep = "/", convert = T) %>%
  mutate(
    monthYear = make_date(year, month),
    metroCat = stringr::str_extract(
  ...
  filter(!is.na(metroCat), !is.na(monthYear)) %>% 
  group_by(metroCat3, monthYear) %>% 
  summarize(
    totalCOVIDDeaths = sum(Deaths),
    .groups = "drop"
  )

### metro-month combined
deaths_Metro_MCD <- inner_join(COVID_Metro_MCD, NC_Metro) %>% 
  filter(monthYear >= make_date(2020, 3))

### division-month MCD
COVID_Division_MCD <- COVID_DivisionRaw_MCD %>% 
  separate(`Month Code`, into = c("year", "month"), sep = "/", convert = T) %>%
  mutate(
    monthYear = make_date(year, month),
    census_division = str_match(
...
### division-month combined
deaths_Division_MCD <- inner_join(COVID_Division_MCD, NC_Division) %>% 
  filter(monthYear >= make_date(2020, 3))
```
...
metroDivisionMonthSimSummary <- simDF %>%
...
metroDivisionMonth <- metroDivisionMonthSimSummary %>%
  # join COVID deaths pre-aggregated from WONDER extract to mitigate suppression
  left_join(
    COVID_MetroDivision,
    by = c("census_division", "metroCat3", "monthYear")
  ) %>%
  mutate(
    # excess and non-covid excess deaths, monthly counts
    # relative to expected, monthly
    across(
      c(starts_with("V") & ends_with("Sum")),
      .fns = c(
        Exc = ~ deaths - .x,
        ExcNC = ~ deaths - .x - totalCOVIDDeaths,
        ExcRel = ~ 100 * (deaths - .x) / .x,
        ExcNCRel = ~ 100 * (deaths - .x - totalCOVIDDeaths) / .x,
        COVIDRel = ~ 100 * totalCOVIDDeaths / .x
        ),
      .names = "{.col}{.fn}"
  )
  ) %>% 
  ...
  ungroup() %>% 
  mutate(
    # running cumulative COVID relative to period total expected
    across(
  ...
  rowwise() %>%
  mutate(
    # running cumulative excess relative to period total expected
    ...
    # running cumulative non-COVID excess relative to period total expected
    excNCCumulativeRelLst = list(purrr::map2_dbl(
      .x = c_across(starts_with("V") & ends_with("ExcNCCumulative")),
      .y = c_across(starts_with("V") & ends_with("SumTotal")),
      ~ 100 * .x / .y
    ))
  ) %>%
  quantile_interval(unlist(excCumulativeRelLst), excCumulativeRel) %>%
  quantile_interval(unlist(excNCCumulativeRelLst), excNCCumulativeRel) %>%
  quantile_interval(
  ...
  quantile_interval(
    c_across(starts_with("V") & ends_with("SumExcRel")), excRel
  ) %>%
  quantile_interval(
    c_across(starts_with("V") & ends_with("SumExcNCRel")), excNCRel
  ) %>%
  quantile_interval(
    c_across(starts_with("V") & ends_with("SumCOVIDRel")), COVIDRel
  ) %>%
  quantile_interval(
  ...
  ungroup() %>%
  select(-c(starts_with("V"), excCumulativeRelLst, excNCCumulativeRelLst))
...
# MCD
metroDivisionMonth_MCD <- metroDivisionMonthSimSummary %>%
  # join COVID deaths pre-aggregated from WONDER extract to mitigate suppression
  left_join(
    COVID_MetroDivision_MCD,
    by = c("census_division", "metroCat3", "monthYear")
  ) %>%
  mutate(
    # excess and non-covid excess deaths, monthly counts
  ...
  select(-c(starts_with("V"), excCumulativeRelLst, excNCCumulativeRelLst))
```
```{r sim data prep division, cache=TRUE}
# MCD
divisionMonth_MCD <- simDF %>%
  # sum to division-month
  group_by(census_division, monthYear) %>%
  summarise(
    # aggregated expected
    across(
  ...
  left_join(
    COVID_Division_MCD,
    by = c("census_division", "monthYear")
  ) %>%
  mutate(
    # excess and non-covid excess deaths, monthly counts
    # relative to expected, monthly
...
```
```{r sim data prep metro, cache=TRUE}
metroMonth_MCD <- simDF %>%
  # sum to metro-month
  group_by(metroCat3, monthYear) %>%
  summarise(
    # aggregated expected
...
metroRegionMonth <- simDF %>%
  ...
  mutate(
    ...
    across(
      ...
      .fns = c(
        ...
        ExcRel = ~ 100 * (totalDeaths - .x) / .x,
        ExcNCRel = ~ 100 * (totalDeaths - .x - totalCOVIDDeaths) / .x,
        COVIDRel = ~ 100 * totalCOVIDDeaths / .x
        ),
      .names = "{.col}{.fn}"
  )
  ...
  mutate(
    ...
    across(
      c(
        starts_with("V") & matches("(Sum|SumExc|SumExcNC)$"),
        totalDeaths, 
        totalCOVIDDeaths
      ),
      .fns = c(
  ...
  mutate(
    ...
    # ratio of excess to covid (non-cumulative)
    across(
      c(starts_with("V") & ends_with("SumExc")),
      .fns = c(COVIDRatio = ~ .x / totalCOVIDDeaths),
      .names = "{.col}{.fn}"
    )
  ) %>%
  rowwise() %>%
  ...
  quantile_interval(
    c_across(starts_with("V") & ends_with("SumExcCOVIDRatio")), excessCOVIDRatio
  ) %>% 
  quantile_interval(
    c_across(starts_with("V") & ends_with("SumCumulative")), expCumulative
  ) %>%
...
metroRegionMonth_MCD <- simDF %>%
  ...
  # add COVID and NC deaths already aggregated from WONDER extract, which have no suppression
  left_join(
    deaths_MetroRegion_MCD, by = c("census_region", "metroCat3", "monthYear")
  ) %>%
  mutate(
    # excess and non-covid excess deaths, monthly counts
    # relative to expected, monthly
...
# MCD
stateMonthCumulative_MCD <- simDF %>%
  # sum to state-month
  group_by(state, monthYear) %>%
    summarise(
    across(
...
metroDivisionMonthLong <- metroDivisionMonth %>% 
  pivot_longer(
    cols = c(
      matches("^(COVIDRelMed|excRelMed|excNCRelMed)$")
      ),
    names_to = "outcome"
  )

# MCD
metroDivisionMonthLong_MCD <- metroDivisionMonth_MCD %>% 
  pivot_longer(
    cols = c(
      matches("^(COVIDRelMed|excRelMed|excNCRelMed)$")
      ),
...
excessVsCOVIDPlot <- metroDivisionMonthLong %>%
  ggplot(
    aes(
      x = monthYear,
      y = value,
  ...
  geom_line() +
  scale_color_manual(
    values = c(colorscaleVik10[2], "grey30", colorscaleVik10[10]),
    labels = c(
      "Reported COVID-19 mortality",
      "Excess natural-cause mortality, excluding reported COVID-19",
      "Excess natural-cause mortality"
    )
  ) +
  scale_linetype_manual(
    values = linetypeCumulative[c(2, 3, 1)],
     labels = c(
      "Reported COVID-19 mortality",
      "Excess natural-cause mortality, excluding reported COVID-19",
      "Excess natural-cause mortality"
  ...
  scale_y_continuous(breaks = seq.int(0, 200, 50)) +
  labs(
    y = "Mortality relative to expected mortality (%)",
    x = NULL,
    linetype = NULL
  ) +
  facet_nested(
...
excessVsCOVIDPlot_MCD <- metroDivisionMonthLong_MCD %>%
  ggplot(
    aes(
      x = monthYear,
      y = value,
...
# MCD
cairo_pdf(
  filename = here::here("figures", paste0(exportName, "_MCD", ".pdf")),
  width = exportWidth,
  height = exportHeight
)
excessVsCOVIDPlot_MCD
dev.off()

ragg::agg_png(
  filename = here::here("figures", paste0(exportName, "_MCD", ".png")),
  width = exportWidth,
  height = exportHeight,
  units = "in",
  res = 300
...
## facet line graphs: division, metro
```{r excess v covid division metro data prep}
divisionMonthLong_MCD <- divisionMonth_MCD %>% 
  pivot_longer(
    cols = c(
      matches("^(COVIDRelMed|excRelMed|excNCRelMed)$")
      ),
...
metroMonthLong_MCD <- metroMonth_MCD %>% 
  pivot_longer(
    cols = c(
      matches("^(COVIDRelMed|excRelMed|excNCRelMed)$")
      ),
...
```
```{r excess v covid division metro plot}
excessVsCOVIDPlotDivision_MCD <- divisionMonthLong_MCD %>%
  ggplot(
    aes(
      x = monthYear,
      y = value,
  ...
  geom_line() +
  scale_color_manual(
    values = c(colorscaleVik10[2], "grey40", colorscaleVik10[9]),
    labels = c(
      "Reported COVID-19 mortality",
      "Excess natural-cause mortality, excluding reported COVID-19",
      "Excess natural-cause mortality"
...
excessVsCOVIDPlotMetro_MCD <- metroMonthLong_MCD %>%
  ggplot(
    aes(
      x = monthYear,
      y = value,
...
excessVsCOVIDPlotCombined_MCD <- excessVsCOVIDPlotDivision_MCD + excessVsCOVIDPlotMetro_MCD +
  plot_layout(
    widths = c(3, 1),
    guides = "collect"
  ) &
...
```
```{r export excess v covid division metro}
exportWidth <- 7
exportHeight <- 7
exportName <- "excessVsCOVIDPlotDivisionMetro"
...
(excessVsCovidFacetCumulativeOut <- gExcessVsCovidFacetCumulative +
  ggh4x::stat_difference(
    ...
    levels = c(
      "Reported COVID-19 mortality > excess natural-cause mortality",
      "Excess natural-cause mortality > reported COVID-19 mortality",
      NA
    ),
    alpha = .75
  ) +
  geom_line(
    aes(
      ...
      color = "Excess natural-cause mortality (cumulative)",
      linetype = "Excess natural-cause mortality (cumulative)"
    ),
    linewidth = .35
  ) +
  geom_line(
    aes(
      ...
      color = "Reported COVID-19 mortality (cumulative)",
      linetype = "Reported COVID-19 mortality (cumulative)"
    ),
    linewidth = .35
  ) +
  scale_color_discrete_diverging(
  ...
  labs(
    y = "Mortality relative to total expected mortality (%)",
    x = NULL,
    color = NULL,
    linetype = NULL,
    fill = NULL
...
# MCD
gExcessVsCovidFacetCumulative_MCD <- ggplot(
  metroDivisionMonth_MCD,
  aes(
    x = monthYear
  )
)

(excessVsCovidFacetCumulativeOut_MCD <- gExcessVsCovidFacetCumulative_MCD +
  ggh4x::stat_difference(
    aes(
      ymax = COVIDCumulativeRelMed,
      ymin = excCumulativeRelMed
...
excessVsCovidFacetCumulativeOut_MCD
dev.off()
...
dev.off()
```
## metro + division line graphs
```{r cumulative excess vs covid metro division}
# division
gExcessVsCovidFacetCumulativeDivision <- ggplot(
  divisionMonth_MCD,
  aes(
    x = monthYear
  )
...
excessVsCovidFacetCumulativeDivisionPlot <- gExcessVsCovidFacetCumulativeDivision +
  ggh4x::stat_difference(
    aes(
      ymax = COVIDCumulativeRelMed,
      ymin = excCumulativeRelMed
  ...
  scale_y_continuous(breaks = seq.int(0, 20, 5)) +
  coord_cartesian(
    ylim = c(0, 24),
    expand = FALSE,
    clip = "off"
...
# metro
gExcessVsCovidFacetCumulativeMetro <- ggplot(
  metroMonth_MCD,
  aes(
    x = monthYear
  )
...
excessVsCovidFacetCumulativeMetroPlot <- gExcessVsCovidFacetCumulativeMetro +
  ggh4x::stat_difference(
    aes(
      ymax = COVIDCumulativeRelMed,
      ymin = excCumulativeRelMed
...
# combined
excessVsCovidFacetCumulativeCombinedPlot <- excessVsCovidFacetCumulativeDivisionPlot + excessVsCovidFacetCumulativeMetroPlot +
  plot_layout(
    widths = c(3, 1),
    guides = "collect"
  ) &
...
```
```{r export cumulative excess vs covid metro division}
exportWidth <- 7
exportHeight <- 7
exportName <- "excess_vs_covid_cumulative_facet_division_metro"
...
excessVsCovidFacetCumulativeCombinedPlot
dev.off()
...
(geofacetCOVIDOut <- gCOVIDGeofacet +
  ggh4x::stat_difference(
    ...
    levels = c(
      "Reported COVID-19 mortality exceeds excess natural-cause mortality",
      "Excess natural-cause mortality exceeds reported COVID-19 mortality",
      NA
    ),
    alpha = .75
  ) +
...
# MCD
gCOVIDGeofacet_MCD <- ggplot(
  stateMonthCumulative_MCD,
  aes(
    x = monthYear
  )
)

(geofacetCOVIDOut_MCD <- gCOVIDGeofacet_MCD +
  ggh4x::stat_difference(
    aes(
      ymax = COVIDCumulativeRelMed,
      ymin = excCumulativeRelMed
...
# MCD
plotMetroRegionMonth_MCD <- metroRegionMonth_MCD %>%
  filter(
    excCumulativeMed > 100,
    totalCOVIDDeathsCumulative > 100
  ) %>% 
...
plotMetroRegionMonthLastObs_MCD <- plotMetroRegionMonth_MCD %>% 
  filter(monthYear == max(monthYear))
```
...
excessCOVIDRatioCumulativePlot <- gExcessCOVIDRatioCumulative +
  ...
  labs(
    y = "Ratio of cumulative excess natural-cause mortality to cumulative reported COVID-19 mortality",
    x = NULL,
    color = "group",
    linetype = "group"
  ) +
...
# MCD
gExcessCOVIDRatioCumulative_MCD <- ggplot(
  plotMetroRegionMonth_MCD,
  aes(
    x = monthYear,
    y = excessCOVIDRatioCumulativeMed,
    color = census_region,
...
excessCOVIDRatioCumulativePlot_MCD <- gExcessCOVIDRatioCumulative_MCD +
  geom_hline(
      yintercept = 1,
      color = "grey50",
      linewidth = .2
  ...
  geom_point(
    data = plotMetroRegionMonthLastObs_MCD,
    shape = 23,
    color = "white",
    stroke = .1
  ) +
...
excessCOVIDRatioCumulativeLabelMargin_MCD <- gExcessCOVIDRatioCumulative_MCD +
  geom_text_repel(
    aes(label = labelMargin),
    size = 2.5,
    segment.size = .3,
...
(
  excessCOVIDRatioCumulativeOut_MCD <- excessCOVIDRatioCumulativePlot_MCD +
    excessCOVIDRatioCumulativeLabelMargin_MCD +
    gLegend +
    plot_layout(design = layout)
)
```
...
excessCOVIDRatioCumulativeOut_MCD
dev.off()
...
dev.off()
```
```{r covid-excess ratio region plot non-cumulative, include=TRUE}
gExcessCOVIDRatio <- ggplot(
  plotMetroRegionMonth,
  aes(
    x = monthYear,
    y = excessCOVIDRatioMed,
    color = census_region,
    fill = census_region,
    linetype = metroCat3
  )
...
excessCOVIDRatioPlot <- gExcessCOVIDRatio +
  geom_hline(
      yintercept = 1,
      color = "grey50",
      linewidth = .2
  ...
  scale_y_continuous(
    trans = scales::pseudo_log_trans(base = 2, sigma = 10),
    breaks = seq.int(-3, 10)
  ) +
  coord_cartesian(
    ylim = c(-3, 9),
  ...
  labs(
    y = "Ratio of excess natural-cause mortality to reported COVID-19 mortality",
    x = NULL,
    color = "group",
    linetype = "group"
  ) +
...
excessCOVIDRatioLabelMargin <- gExcessCOVIDRatio +
  geom_text_repel(
    aes(label = labelMargin),
    size = 2.5,
    segment.size = .3,
...
(
  excessCOVIDRatioOut <- excessCOVIDRatioPlot +
    excessCOVIDRatioLabelMargin +
    gLegend +
    plot_layout(design = layout)
)
```
```{r export non-cumulative ratio}
exportWidth <- 7
exportHeight <- 7
exportName <- "excess_to_covid_ratio"
