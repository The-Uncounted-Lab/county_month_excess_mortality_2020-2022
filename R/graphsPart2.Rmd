...
finalSimsFIPS <- finalSimsFIPS %>%
  rename(deathsOriginal = deaths,
         deaths = imputedDeaths) %>%
...
trendPlots <- finalSimsFIPS %>%
  drop_na() %>%
  filter(monthYear>=make_date(2019,1,1)) %>%
  group_by(metroCat,BEARegion,monthYear) %>%
  summarize(excDeathRate = ((sum(deaths)-sum(expDeathsMed))/sum(pop))*100000) %>%
  ungroup() %>%
  ggplot() +
  ...
  geom_line(mapping=aes(x=monthYear,y=excDeathRate,group=metroCat,color=metroCat)) +
...
BEABarsData2020 <- finalSimsFIPS %>%
  drop_na() %>%
  filter(year==2020) %>%
  group_by(metroCat,BEARegion,year) %>%
...
stackBarsBEAPlot2020 <- BEABarsData2020 %>%
  ggplot() +
  geom_bar(mapping=aes(x=BEARegion,y=excDeaths,fill=metroCat),
...
BEABarsData2020PC <- finalSimsFIPS %>%
  drop_na() %>%
  filter(year==2020) %>%
  group_by(metroCat,BEARegion,FIPSCode,year) %>%
...
largestCounties <- finalSimsFIPS %>%
  drop_na() %>%
  group_by(BEARegion,FIPSCode,metroCat) %>%
  summarize(popMean = mean(pop)) %>%
  ungroup() %>%
  group_by(BEARegion,metroCat) %>%
...
largestCountiesPlots <- largestCountiesData %>%
  ...
  facet_grid(rows = vars(BEARegion), cols=vars(metroCat)) +
  labs(x='',
