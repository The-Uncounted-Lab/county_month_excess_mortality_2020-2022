...
arrow::write_feather(plotData, here(outDir, "plotDataRelative.feather"))
```
...
```{r plot data prep}
# plotData <- arrow::read_feather(here(outDir, "plotDataRelative.feather"))
...
plotDataRelativeHeatmap <- plotDataRelative %>%
  ...
  mutate(
    ...
    excDeathsRelativeNoInf = if_else(
      monthYear >= date("2020-03-01"),
      excDeathsRelativeNoInf,
      NA_real_
    ),
...
gHeatmapOrdered +
  ...
  ggh4x::facet_nested(
    rows = vars(census_region, metroStatus),
    scales = "free_y",
    space = "free_y",
