...
here::i_am('R/exMortPerformanceNC.Rmd')
...
load(here::here('R','RObjects','modelBaseNCLT.RData'))
load(here::here('R','RObjects','modelRandomTimeNCLT.RData'))
load(here::here('R','RObjects','modelBaseNCLF.RData'))
...
RMSEs <- tibble(Performance=c('Training Data (2014-2018)',
                                  'Test Data (2019)',
                                  'Overall'),
                    modelBase=c(RMSE(filter(drop_na(NCData),year<2019)$CDR,
                                     predict(modelBaseLT)),
                                RMSE(filter(drop_na(NCData),year==2019)$CDR,
                                     predict(modelBaseLT,filter(drop_na(NCData),year==2019),
                                             allow.new.levels=T)),
                                RMSE(filter(drop_na(NCData),year<=2019)$CDR,
                                     predict(modelBaseLT,filter(drop_na(NCData),year<=2019),
                                             allow.new.levels=T))),
                    modelRandomTime=c(RMSE(filter(drop_na(NCData),year<2019)$CDR,
                                           predict(modelRandomTimeLT)),
                                      RMSE(filter(drop_na(NCData),year==2019)$CDR,
                                           predict(modelRandomTimeLT,filter(drop_na(NCData),year==2019),
                                                   allow.new.levels=T)),
                                      RMSE(filter(drop_na(NCData),year<=2019)$CDR,
                                           predict(modelRandomTimeLT,filter(drop_na(NCData),year<=2019),
                                                   allow.new.levels=T))))
...
yearlyData <- simDataMonthly %>% 
  ...
  left_join(dplyr::select(NCYearlyData,FIPSCode,year,deaths),by=c('FIPSCode','year')) %>%
  group_by(FIPSCode) %>%
...
yearlyData %>%
  write_csv(file=here::here(outDir,'problematicFIPSNCData.csv'))
...
simulations <- simDataMonthly %>%
  ...
  left_join(dplyr::select(NCData,FIPSCode,month,year,deaths), 
...
```{r, echo=F, message=F, warning=F, fig.align='center', fig.width=15, fig.height=8}
...
for (countyGroup in countyGroups) {
  ...
  ggsave(here::here('figures',paste0('countyPlotsNC',as.character(i),'.pdf')),
...
cat(countiesIndex, file = here::here('figures','plotIndexNC.txt'))
